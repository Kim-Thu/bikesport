...
export function SearchForm({ placeholder = "Tìm kiếm..." }: SearchFormProps) {
  ...
  return (
    <form className="flex h-11 min-w-0 flex-1 overflow-hidden rounded-md border border-gray-300 bg-white" role="search" onSubmit={handleSubmit}>
      <label htmlFor="site-search" className="sr-only">Tìm kiếm</label>
      <input id="site-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={placeholder} className="min-w-0 flex-1 border-0 px-4 outline-none placeholder:text-gray-400" />
