  ...
  return (
    <article className={cn("group flex h-full flex-col overflow-hidden rounded-lg border border-gray-100 bg-white transition-colors hover:border-blue-200", className)}>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4">
            ...
            <div className="space-y-2 text-xs text-gray-500">
              {metaItems.map((item) => (
                <div key={`${item.icon ?? "meta"}-${item.text}`} className="flex items-start gap-2">
                  <Icon name={item.icon} className="mt-0.5 h-4 w-4 shrink-0 text-blue-700" />
                  <span>{item.text}</span>
                </div>
              ))}
            </div>
        ...
        {actionLabel ? <CLink href={href} className="mt-auto text-xs font-semibold text-blue-700">{actionLabel}</CLink> : null}
      </div>
    </article>
  );
}
